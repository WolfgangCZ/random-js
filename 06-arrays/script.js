...
console.log("name2");
console.log(test1.name2);
console.log("name3");
console.log(test2.name3);
...
const fieldPlayers = [...players1.slice(1), ...players2.slice(1)];
...
{
  console.log(`Goal ${name}: ${game.scored[name]}`)
}
// 2. Use a loop to calculate the average odd and log it to the console 
//(We already studied how to calculate averages, you can go check if you don't remember)

let sum = 0;
for (const num in game.odds)
{
  sum += Number(game.odds[num]);
  console.log(game.odds[num]);
}
console.log(sum/3);
...
{
  console.log(`Odd of ${("victory of " && game[odd]) || "draw"} is ${game.odds[odd]}`)
}
...
gameEvents.forEach((min,value) => {
  console.log(Number(min) < 45 ? `[FIRST HALF]: ${value}` : `[SECOND HALF]: ${value}`)
});
*/
...
The input will come from a textarea inserted into the DOM (see code below), and conversion will happen 
...
//let str = prompt("write input")
let str = "delayed_departure"
str = str.toLowerCase();
str = str.trim();
...
{
  const firstLetter = str_arr[i].slice(0,1).toUpperCase();
  const rest = str_arr[i].slice(1);
  str_arr[i] = firstLetter + rest;
